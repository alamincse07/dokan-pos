...
class Register extends XModel
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('password', 'length', 'min'=>6,'message'=>'Password must at least 6 characters long'),
            array('password','match','pattern'=>'/\d/', 'message'=>'Password must contain at least one Numeric digit.'),
            array('password','match','pattern'=>'/(?=.*[A-Z])/', 'message'=>'Password must contain at least one Capital letter.'),

            //array('password','match','pattern'=>'/^(?=.*\d)(?=.*[A-Z]).{6,25}$/', 'message'=>'Password must contain at least one Capital letter,one Number & minimum 6 character.'),
    ...
    public function relations()
    {
        ...
        return array(
            'accesses' => array(self::HAS_MANY, 'Access', 'Register_id'),
        );
    }
    ...
    public function getRegisterComboService($active=1)#changed here #active=true
    {
    ...
    #30-01-14
    public function getusernames($Register_id,$active=1){
        return CHtml::listData($this->findAll("is_active=:active AND id!=:id",array(':active'=>$active,':id'=>$Register_id)),'id','username');
    }
    #30-01-14


    //add directory for new Register
