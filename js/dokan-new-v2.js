  ...
  if ($("#paikari_chk").is(":checked")) {
    ...
    for (var j = 1; j <= pair2; j++) {
      $.ajax({
        dataType: "json",
        type: "POST",
        url: SITE_URL + "SimpleAjax/Sold?category=common",
        data: form_data,
        success: function (data) {
          var total = 0;
          total_in++;
...
function SellItem(form_data) {
  $(".last_addedc").html("");

  $(".last_added").html("");
  var html = "যোগের পূর্বে সঠিক তথ্য দিন ";

  var total_taka = $("#totalItemPrice").val();
  var total_in = 0;
  var isPrint = $("#isPrint").is(":checked");
  var pair2 = 1; //$("#multipair").val();
  for (var j = 1; j <= pair2; j++) {
    $.ajax({
      ...
      success: function (data) {
        if (data.status == "success") {
          ...
          $("#total_" + data.category + "_taka").html(parseInt(etaka, 10) + parseInt(data.price, 10));
