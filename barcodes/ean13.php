...
function ean13_barcode($code, $scale = 1, $height = 0){
  ...
  if ($scale<1) $scale=2;
  $height=(int)($height);
  if ($height<1) $height=(int)$scale * 25;
  ...
  $im=imagecreate($total_x, $height+10);
  /* create two images */
...
imagestring($im, 4, 5, ($height1+10), "SAlIMAR-BELL-D-46-5-8-10-13", $col_bar );
  ...
  while (list($n, $v)=each($chars)){
    if (trim($v)){
        ...
        @imagettftext($im, $fontsize, 0, $space['left']+($scale*$inf[0])+2,$fontheight1, $col_text, $font, $inf[2]);
    }
  }
